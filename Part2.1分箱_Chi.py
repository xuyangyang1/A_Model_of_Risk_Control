...
def AssignGroup(x, bin):
    ...
    if x<=min(bin):
        ...
        # Use +inf so overflow stays after every finite split point when sorted.
        # A fixed sentinel like 10e10 sorts before large real values (e.g. ms timestamps)
        # and silently corrupts ChiMerge bucket ordering.
        return float('inf')
    else:
        for i in range(N-1):
            if bin[i] < x <= bin[i+1]:
